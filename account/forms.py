...
class RegistrationForm(forms.ModelForm):
    ...
    password = forms.CharField(max_length=8, widget=forms.PasswordInput, required=True)

    password_confirm = forms.CharField(max_length=8, widget=forms.PasswordInput, required=True)
    ...
    def clean_email(self):
        ...
        if User.objects.filter(email=email).exists():
            raise forms.ValidationError('Пользователь уже зарегился ')
        return email
...
class ChangePasswordForm(forms.Form):
    ...
    def clean_old_pass(self):
        ...
        if not user.check_password(old_pass):
            raise forms.ValidationError('Укажите верный пароля')
        return old_pass
